...
{
    [super viewDidLoad];
    NSLog(@"SVTabbarController页面");

    //测试
    SVTestViewCtrl* testCtrl = [SVTestViewCtrl new];
    [self addChildViewController:testCtrl
                       imageName:@"tabbar_test"
                           title:@"测试"];
    //结果
    SVResultViewCtrl* resultCtrl = [SVResultViewCtrl new];
    [self addChildViewController:resultCtrl
                       imageName:@"tabbar_result"
                           title:@"结果"];
    //设置
                         ...
                         title:(NSString*)title
{
    ...
    [childCtrl.tabBarItem setTitleTextAttributes:@{
        NSForegroundColorAttributeName : [UIColor colorWithRed:29 / 255.0
                                                         green:145 / 255.0
                                                          blue:226 / 255.0
                                                         alpha:1.0]
    ...
    UINavigationController* navCtrl =
        [[UINavigationController alloc] initWithRootViewController:childCtrl];
    //添加控制器
...
- (void)setShadowView
{
    ...
    [UIView animateWithDuration:
        ...
        shadowView.backgroundColor = [UIColor colorWithWhite:0.3 alpha:1];

        if (!_alertView) {
            _alertView = [[AlertView alloc]
                initWithFrame:CGRectMake(FITWIDTH(20), FITWIDTH(140),
                                  shadowView.frame.size.width - FITWIDTH(40),
                                  FITWIDTH(220))
