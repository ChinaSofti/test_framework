...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier
{
    if ([super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [_bgdBtn addTarget:self action:@selector(bgdBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_bgdBtn];
        
        CGFloat imgViewWAndH = kViewH(_bgdBtn) - 3 * kViewX(_bgdBtn);
        
        _imgView = [[UIImageView alloc] initWithFrame:CGRectMake(kMargin * 2, (kCellH - imgViewWAndH) * 0.5, imgViewWAndH, imgViewWAndH)];
        [_bgdBtn addSubview:_imgView];
        
        _rightImgView = [[UIImageView alloc] initWithFrame:CGRectMake(kViewW(_bgdBtn) - imgViewWAndH - kMargin, kViewY(_imgView), imgViewWAndH, imgViewWAndH)];
        [_bgdBtn addSubview:_rightImgView];
        
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(kViewR(_imgView) + 30, kViewY(_imgView), kViewX(_rightImgView) - kViewR(_imgView) - 30, imgViewWAndH)];
        //初始化透明度
        _bgdBtn.alpha = 0.5;
...
- (void)bgdBtnClick:(UIButton*)btn
{
    ...
    if (btn.selected) {
        ...
        _bgdBtn.layer.borderColor = [CTViewTools colorWithImg:img point:CGPointMake(img.size.width * 0.75, img.size.height * 0.75)].CGColor;
        //选中后透明度设置为1.0
        _bgdBtn.alpha = 1;
    }
    else {
