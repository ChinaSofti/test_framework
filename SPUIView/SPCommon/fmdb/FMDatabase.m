...
{
    int err =
    sqlite3_open_v2 ((_databasePath ? [_databasePath fileSystemRepresentation] : ":memory:"), &_db,
                     flags, NULL /* Name of VFS module to use */);
    if (err != SQLITE_OK)
    {
        NSLog (@"error opening!: %d", err);
        return NO;
    ...
    for (NSUInteger i = 0; i < length; ++i)
    {
        ...
        if (last == '%')
        {
            ...
            case 'g':
                ...
                arg = [NSNumber numberWithFloat:va_arg (args, double)];
                break;
            case 'l':
                i++;
                if (i < length)
             ...
             orVAList:(va_list)args
{
    ...
    if (!pStmt)
    {
        ...
        do
        {
            ...
            else if (SQLITE_OK != rc)
            {
                ...
                if (outErr)
                {
                    *outErr = [NSError errorWithDomain:[NSString stringWithUTF8String:sqlite3_errmsg (_db)]
                                                  code:rc
                                              userInfo:nil];
                }
    ...
    {
        NSLog (@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode,
               sqlite3_errmsg (_db));
        NSLog (@"DB Query: %@", sql);
    }
...
- (BOOL)executeUpdate:(NSString *)sql, ...
{
    ...
    BOOL result =
    [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:nil orVAList:args];
...
{
    return
    [self executeUpdate:sql error:nil withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (BOOL)executeUpdate:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments
{
    return
    [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}
...
- (BOOL)rollbackToSavePointWithName:(NSString *)name error:(NSError **)outErr
{
    ...
    BOOL worked =
    [self executeUpdate:[NSString stringWithFormat:@"rollback transaction to savepoint '%@';", name]];
 ...
 * seemed to never be called, so we use _openFunctions to store the values instead. */
    ...
    sqlite3_create_function ([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void *)b,
                             &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function ([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8,
                             (__bridge void *)b, &FMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
